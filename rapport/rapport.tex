...
\pretitle{%
  \begin{center}
  \LARGE
  \includegraphics[scale=0.7]{diderot_up.jpg}\\[\bigskipamount]
}
...
\noindent Les noeuds terminaux sont $0$ et $1$.\\
...
\node[state]		   (X1) at (0,4)         {$x_1$};
  ...
  \node[state, below]		   (Y21) at (-4,1)         {$y_2$};
  \node[state, below]		   (Y22) at (-2,1)         {$y_2$};
  \node[state, below]		   (Y23) at (2,1)         {$y_2$};
  \node[state, below]		   (Y24) at (4,1)         {$y_2$};
   \node[state, accepting, below]		   (T1) at (-4.5,-0.5)         {$1$};
   \node[state, accepting, below]		   (F1) at (-3.5,-0.5)         {$0$};
   \node[state, accepting, below]		   (F2) at (-2.5,-0.5)         {$0$};
   \node[state, accepting, below]		   (T2) at (-1.5,-0.5)         {$1$};
   \node[state, accepting, below]		   (F3) at (-0.5,-0.5)         {$0$};
   \node[state, accepting, below]		   (F4) at (0.5,-0.5)         {$0$};
   \node[state, accepting, below]		   (T3) at (1.5,-0.5)         {$1$};
   \node[state, accepting, below]		   (F5) at (2.5,-0.5)         {$0$};
   \node[state, accepting, below]		   (F6) at (3.5,-0.5)         {$0$};
   \node[state, accepting, below]		   (T4) at (4.5,-0.5)         {$1$};
  ...
  \path[->,>=stealth'] 
        ...
        (Y24) edge              node {} (T4);
    \end{tikzpicture}
\caption{BDD de $( x_1 \Leftrightarrow y_1 ) \wedge ( x_2 \Leftrightarrow y_2 )$}
\end{figure}
...
\noindent On remarque certains noeuds sont dupliqués.\\
...
\noindent Le diagramme devient donc :\\
\begin{figure}[h]
\centering
      \begin{tikzpicture}[shorten >=1pt,node distance=1cm]
\node[state]		   (X1) at (0,4)         {$x_1$};
  \node[state]		   (Y11) at (-1.8,3)         {$y_1$};
  \node[state]		   (Y12) at (1.8,3)         {$y_1$};
  \node[state]		   (X2) at (0,2)         {$x_2$};
  \node[state]		   (Y21) at (-2,1)         {$y_2$};
  \node[state]		   (Y22) at (2,1)         {$y_2$};
  \node[state, accepting, below]		   (T) at (0.8,0)         {$1$};
  \node[state, accepting, below]		   (F) at (-0.8,0)         {$0$};
  ...
  \path[->,>=stealth'] 
        ...
        (Y22) edge              node {} (T);
    \end{tikzpicture}
\caption{ROBDD de $( x_1 \Leftrightarrow y_1 ) \wedge ( x_2 \Leftrightarrow y_2 )$}
\end{figure}
...
\noindent On obtient, en passant par notre algorithme et en générant la formule correspondante à la résolution du problème, on obtient le ROBDD suivant :
\newpage
\begin{figure}[h]
\centering
\includegraphics[scale = 0.22]{exemple/queen_4.pdf}
\end{figure}

\noindent On peut lire graphiquement que le problème des 4 reines possède un total de 2 solutions.\\
Voici la représentation sur un échiquier des deux solutions (la branche de gauche correspond à l'échiquier de gauche et la branche de droite à celui de droite) :
\begin{center}
\setchessboard{
showmover=false}
\def\mylist{qa2, qb4, qd3, qc1}
\setchessboard{setpieces=\mylist}
\chessboard[largeboard,maxfield=d4]
\def\mylist{Qa3, Qc4, Qd2, Qb1}
\chessboard[largeboard,maxfield=d4]
\end{center}

\noindent La raison pour laquelle ce Sat-solveur n'est pas utilisé est que, bien que la compléxité de l'algorithme ANYSAT soit linéaire, la fonction BUILD est en compléxité exponentielle.\\
...
\end{tikzpicture}
\caption{Représentation du circuit sous ROBDD d'ordre : $x < y$}
\end{figure}
...
\hspace{5em}
\begin{tikzpicture}[shorten >=1pt,node distance=1cm]
\node 			(N) at (0,2.5)		{Variable : $y$};
  \node[state]		   (X) at (0,1.5)         {$y$};
  \node[state]         (Y) at (-1,0)		 {$0$};
  \node[state]         (Z) at (1,0)		 {$1$};
