...
struct
  ...
  let rec string_of_exp (e : Interface.t exp) (esp : string) : string =
    match e with
    | T b -> string_of_bool b
    | Var n -> "x" ^ Interface.t_to_string n
  ...
  (*let string_of_expression (e : Interface.expression) (esp : string) : string = string_of_exp (Interface.translate e) esp*)

  (*Affiche les exp*)

  let rec getBool (e : Interface.t exp) (tab : bool option array) : (bool option * Interface.t exp) =
    match e with
    | T b -> (Some b, T b)
    | Var i -> begin 
  ...
  (*let getBool_expression (e : Interface.expression) (tab : bool option array) : (bool option * exp) = getBool (Interface.translate e) tab *)
  ...
  let add (i : Interface.t) (l : node) (k : node) : node = 
  ...
  let member (i : 'a) (l : node) (k : node) : bool =
    Hashtbl.mem h (i,l,k) 

  let lookup (i : 'a) (l : node) (k : node) : node =
    Hashtbl.find h (i,l,k)

  let insert (i : 'a) (l : node) (k : node) (u : node) : unit =
  ...
  let build (f : 'a exp) : node = let jkl = ref 0 in
    let tab = Array.init (!n) (function a -> None) in(*Ajouter tableau de comparaison*)
    let rec aux (f : 'a exp) (i : int) : node =
  ...
  (*let build_expression (f : Interface.expression) : node = build (Interface.translate f)*)
  ...
  let restrict (u : node) (j : 'a) (b : bool) : node = 
    let rec aux (u : node) : node =
      if Interface.compare (var u) j < 0 then u
      else if Interface.compare (var u) j > 0 then mk (var u) (aux (low u)) (aux (high u))
  ...
  let satCount (u : node) : int =
    let rec aux (u : node) : int =
      if u = 0 then 0
        ...
        two_power (var (low u) - var u - 1) * aux (low u) +(*Use Compare ?*)
        two_power (var (high u) - var u - 1) * aux (high u)
    in aux u

  let rec anySat (u : node) : ('a* bool) list =
    assert (u != 0);
    if u = 1 then []
    else if low u = 0 then (var u, true) :: (anySat (high u))
    else (var u, false) :: (anySat (low u))

  let allSat (u : node) : ('a* bool) list list =
    let rec aux (u : node) (l : ('a* bool) list) (r : ('a* bool) list list) : ('a* bool) list list =
      if u = 0 then r
      else if u = 1 then l::r
      else aux (high u) ((var u, true)::l) (aux (low u) ((var u, false)::l) r)
    in aux u [] []
  ...
  let aff (n : node) : unit =
    let rec aux (n : node) (esp : string) : unit =
      ...
      if not(unite n) then 
        begin 
          print_endline (":" ^ (Interface.t_to_string (var n))); 
          aux (low n) (esp ^ " |"); 
          print_endline ""; 
          aux (high n) (esp ^ "  ")
        end
  ...
  let main (e : exp) (i : int) : unit =
	n := i; initT();
	initH();
	print_endline (string_of_exp e "");
	print_endline "Start Build ..."; 
	let x = build e in aff x;
	print_endline (string_of_int (satCount x))

  let main_expression (e : Interface.expression) (i : int) : unit = main (Interface.translate e) i

end

(*Test*)

module Test : InterfaceROBDD = struct
  (*type var = int*)
  type t = int
...
let main (e : Test.expression) (i : int) : unit = T.main_expression e i
